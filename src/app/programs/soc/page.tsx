...
import { Shield, Eye, AlertTriangle, Activity, Search, FileText, ArrowRight, ShieldCheck } from "lucide-react";
    ...
    <div ref={containerRef} className="opacity-0 relative min-h-screen pt-32 pb-20 overflow-hidden font-mono bg-black text-white">
      ...
      <main className="relative z-10 w-full max-w-6xl mx-auto px-6 flex flex-col items-center gap-20">
        <section className="flex flex-col items-center text-center max-w-3xl space-y-6">
          <SectionBadge ref={badgeRef} text="PHASE 2 · SPECIALIZATION" icon={Shield} />
          <h1 ref={headingRef} className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter text-white uppercase leading-tight">
            SOC Analyst <span className="text-red-500">Track</span>
          </h1>
        </section>
        ...
        <div ref={contentRef} className="opacity-0 flex flex-col items-center gap-20 w-full">
          ...
          <section className="w-full flex flex-col gap-16">
            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center gap-4 border-b-2 border-white/20 pb-4">
                ...
                <div>
                  <h2 className="text-2xl font-black uppercase tracking-tight text-white">SIEM & Log Management</h2>
                  <p className="text-sm text-gray-400 uppercase tracking-widest font-bold">Weeks 13 - 16</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModuleCard icon={<Activity className="text-red-500" size={24} />} title="Splunk Core" desc="Search, dashboards, alerts, SPL query language" />
                <ModuleCard icon={<Shield className="text-red-500" size={24} />} title="Microsoft Sentinel" desc="KQL, Analytic rules, Playbooks, Workbooks" />
                <ModuleCard icon={<Eye className="text-red-500" size={24} />} title="Log Collection & Parsing" desc="Syslog, WEF, Beats, Fluentd, index management" />
                <ModuleCard icon={<AlertTriangle className="text-red-500" size={24} />} title="Alert Tuning & Correlation" desc="Reducing noise, correlation rules, true/false positives" />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center gap-4 border-b-2 border-white/20 pb-4">
                ...
                <div>
                  <h2 className="text-2xl font-black uppercase tracking-tight text-white">Threat Hunting & MITRE ATT&CK</h2>
                  <p className="text-sm text-gray-400 uppercase tracking-widest font-bold">Weeks 17 - 20</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModuleCard icon={<Search className="text-red-500" size={24} />} title="Threat Hunting Methodology" desc="Hypothesis-driven hunting, IoC vs IoA, hunting loops" />
                <ModuleCard icon={<Shield className="text-red-500" size={24} />} title="MITRE ATT&CK Framework" desc="Tactics, techniques, procedure mapping and detection" />
                <ModuleCard icon={<Activity className="text-red-500" size={24} />} title="Endpoint Detection (EDR)" desc="CrowdStrike, Defender for Endpoint, process trees" />
                <ModuleCard icon={<Eye className="text-red-500" size={24} />} title="Network Traffic Analysis" desc="Zeek, Suricata, NSM, anomaly detection" />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ModuleCard icon={<AlertTriangle className="text-red-500" size={24} />} title="Incident Response Lifecycle" desc="PICERL framework, escalation, war-room management" />
                <ModuleCard icon={<FileText className="text-red-500" size={24} />} title="Playbook Development" desc="SOAR, automated response, runbook authoring" />
                <ModuleCard icon={<Shield className="text-red-500" size={24} />} title="Threat Intelligence" desc="OSINT, threat feeds, CTI integration into SIEM" />
                <ModuleCard icon={<ShieldCheck className="text-red-500" size={24} />} title="Live-Fire SOC Capstone" desc="48-Hour SOC simulation: detect, triage, respond, report" highlight />
              </div>
            </div>
          </section>
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-white font-bold text-sm uppercase tracking-wide mb-1">Zharnyx SOC Analyst Certification</p>
              <p className="text-gray-400 text-sm leading-relaxed">
                This track is engineered to prepare you for the upcoming <span className="text-white font-bold">ZSA</span> (Zharnyx SOC Analyst) certification — a 100% hands-on, no-MCQ practical exam. Currently in active development.
              </p>
            </div>
          ...
          <section className="w-full bg-red-600/10 border-2 border-red-600 p-8 md:p-12 text-center shadow-[8px_8px_0px_0px_rgba(220,38,38,1)] flex flex-col items-center gap-8 mt-4">
            <div>
              ...
              <p className="text-red-400 font-bold uppercase tracking-widest text-sm">Enroll in the SOC Analyst Track</p>
            </div>
