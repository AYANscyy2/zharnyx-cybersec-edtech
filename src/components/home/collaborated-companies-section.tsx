...
export function CollaboratedCompaniesSection() {
  return (
    <section className="w-full bg-[#040404] py-20 overflow-hidden relative">
      <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10 pointer-events-none mix-blend-overlay"></div>
      
      <div className="w-full flex justify-center mb-16 relative z-10">
        <div className="rounded-full border border-red-500/20 bg-red-500/10 px-4 py-2 flex items-center gap-3">
        ...
        <div className="flex w-max" style={{ animation: "scroll-fast 30s linear infinite" }}>
          {EXTENDED.map((company, idx) => (
            <div key={idx} className="flex items-center mx-8">
              <span className="text-4xl md:text-5xl lg:text-6xl font-bold uppercase text-white/80 hover:text-white transition-colors cursor-default whitespace-nowrap tracking-tight">
                {company}
              </span>
              <span className="text-white/20 mx-12 text-3xl font-bold">•</span>
            </div>
          ))}
        </div>
      </div>
