...
export function StudentSidebar({ activeSection, onSectionChange, userId }: StudentSidebarProps) {
  ...
  return (
    <Sidebar className="border-r border-white/10 bg-black/90 text-white" collapsible="icon">
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="text-gray-500 font-mono">Learning</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {items.map((item) => (
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <button>
                      ...
                      {item.isLink && <span className="ml-auto text-xs text-gray-500">↗</span>}
                    </button>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
